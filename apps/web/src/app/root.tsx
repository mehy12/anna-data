...
// FIX: Removed the failing import for './+types/root'
 ...
 */
export function ErrorBoundary({ error }: ErrorBoundaryProps) { // <-- FIX: Used the new interface
  return <SharedErrorBoundary isOpen={true} />;
}

function InternalErrorBoundary({ error: errorArg }: ErrorBoundaryProps) { // <-- FIX: Used the new interface
...
export function Layout({ children }: { children: ReactNode }) {
  ...
  return (
    <html lang="en">
      ...
      <body>
        <ClientOnly loader={() => children} />import {
  Links,
  Meta,
  Outlet,
  Scripts,
